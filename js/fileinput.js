...
function fileInput(fi_container_class, fi_button_class, fi_filename_class, fi_button_text) {
	...
	fi_button_text		=	fi_button_text		||	'Sélectionner l\'image principale'; // Text inside the button
	...
	// String to append
	var fi_str = '<div class="'+fi_container_class+'"><div class="'+fi_filename_class+'">'+fi_button_text+'</div></div>';
	...
	// Button: action
	$('.'+fi_filename_class).on('click', function() {
	...
	fi_file.on('change', function() {
		...
		$('.'+fi_container_class).each(function() {
			...
			if(fi_real_name == fi_fake_name) {
				...
				$("."+fi_filename_class).css({color:'black'});
			}
		});
	});
}